...
JNI_METHOD(jobject, attemptReconnectToLastPlayer)(JNIEnv * env, jclass)
{
    ...
    {
        // GetTargetCastingPlayer returned null, try reading from persistent storage
        ChipLogProgress(AppServer, "ManualCommissioningHelper::attemptReconnectToLastPlayer() No target player, reading from CastingStore");
        
        std::vector<matter::casting::core::CastingPlayer> cachedPlayers = matter::casting::support::CastingStore::GetInstance()->ReadAll();
        
        if (!cachedPlayers.empty())
        {
            ChipLogProgress(AppServer, "ManualCommissioningHelper::attemptReconnectToLastPlayer() Found %lu cached player(s), attempting to reconnect to first one",
                            static_cast<unsigned long>(cachedPlayers.size()));
            
            // Use the first cached player
            matter::casting::core::CastingPlayer * firstPlayer = &cachedPlayers[0];
            ...
            firstPlayer->VerifyOrEstablishConnection(callbacks, matter::casting::core::kCommissioningWindowTimeoutSec);
            
            return matter::casting::support::convertMatterErrorFromCppToJava(CHIP_NO_ERROR);
        }
        else
        {
            ChipLogError(AppServer, "ManualCommissioningHelper::attemptReconnectToLastPlayer() No cached players found in CastingStore");
            return matter::casting::support::convertMatterErrorFromCppToJava(CHIP_ERROR_INCORRECT_STATE);
        }
    }
    else
    {
        ChipLogProgress(AppServer, "ManualCommissioningHelper::attemptReconnectToLastPlayer() Found target player, verifying/establishing connection");
...
// Global flag to track verification result (set by callback)
static bool gVerificationResult = false;
static bool gVerificationComplete = false;

JNI_METHOD(jboolean, verifyConnectionAlive)(JNIEnv * env, jclass)
{
    chip::DeviceLayer::StackLock lock;
    ChipLogProgress(AppServer, "ManualCommissioningHelper::verifyConnectionAlive() called");
    
    // Check if we have a commissioned player
    if (gCommissionedVideoPlayer == nullptr)
    {
        ChipLogProgress(AppServer, "ManualCommissioningHelper::verifyConnectionAlive() No commissioned player, returning false");
        return JNI_FALSE;
    }
    
    // Check if the player is still initialized
    if (!gCommissionedVideoPlayer->IsInitialized())
    {
        ChipLogProgress(AppServer, "ManualCommissioningHelper::verifyConnectionAlive() Player not initialized, returning false");
        return JNI_FALSE;
    }
    
    // Get the CastingServer instance
    CastingServer * castingServer = CastingServer::GetInstance();
    if (castingServer == nullptr)
    {
        ChipLogError(AppServer, "ManualCommissioningHelper::verifyConnectionAlive() CastingServer is null");
        return JNI_FALSE;
    }
    
    // Get the active target video player from CastingServer
    ...
    {
        ChipLogProgress(AppServer, "ManualCommissioningHelper::verifyConnectionAlive() No active player in CastingServer");
        return JNI_FALSE;
    }
    
    // Try to find an endpoint we can use to verify connection
    TargetEndpointInfo * endpoints = activePlayer->GetEndpoints();
    if (endpoints == nullptr)
    {
        ChipLogProgress(AppServer, "ManualCommissioningHelper::verifyConnectionAlive() No endpoints available");
        return JNI_FALSE;
    }
    
    // Find an endpoint with KeypadInput cluster (should exist on most TVs)
    TargetEndpointInfo * testEndpoint = nullptr;
    for (size_t i = 0; i < kMaxNumberOfEndpoints && endpoints[i].IsInitialized(); i++)
    {
    ...
    gVerificationResult = false;
    gVerificationComplete = false;
    
    // Send a simple KeypadInput command to verify connection
    // Using a no-op key code that shouldn't affect the TV
    chip::app::Clusters::KeypadInput::CECKeyCodeEnum testKeyCode = 
        chip::app::Clusters::KeypadInput::CECKeyCodeEnum::kRootMenu;
    
    ChipLogProgress(AppServer, "ManualCommissioningHelper::verifyConnectionAlive() Sending heartbeat to endpoint %d",
                    testEndpoint->GetEndpointId());
    
    CHIP_ERROR err = castingServer->KeypadInput_SendKey(
        testEndpoint,
        testKeyCode,
        [](CHIP_ERROR err) {
            gVerificationComplete = true;
            if (err == CHIP_NO_ERROR)
            {
                ChipLogProgress(AppServer, "ManualCommissioningHelper::verifyConnectionAlive() Heartbeat SUCCESS - connection alive");
                gVerificationResult = true;
            }
            else
            {
                ChipLogError(AppServer, "ManualCommissioningHelper::verifyConnectionAlive() Heartbeat FAILED: %" CHIP_ERROR_FORMAT, err.Format());
                gVerificationResult = false;
            }
        });
    ...
    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(AppServer, "ManualCommissioningHelper::verifyConnectionAlive() Failed to send heartbeat: %" CHIP_ERROR_FORMAT, err.Format());
        return JNI_FALSE;
    }
    
    // Wait for the callback with a timeout (max 5 seconds)
    int waitCount = 0;
    const int maxWait = 50; // 50 * 100ms = 5 seconds
    while (!gVerificationComplete && waitCount < maxWait)
    {
        // Process events to allow callback to complete
        chip::DeviceLayer::PlatformMgr().ScheduleWork([](intptr_t) {}, 0);
        usleep(100000); // 100ms
        waitCount++;
    }
    
    if (!gVerificationComplete)
    {
        ChipLogError(AppServer, "ManualCommissioningHelper::verifyConnectionAlive() Heartbeat timed out - assuming disconnected");
        return JNI_FALSE;
    }
    
    ChipLogProgress(AppServer, "ManualCommissioningHelper::verifyConnectionAlive() returning: %s", 
                    gVerificationResult ? "true (connected)" : "false (disconnected)");
    
    return gVerificationResult ? JNI_TRUE : JNI_FALSE;
}

JNI_METHOD(void, clearCommissionedPlayer)(JNIEnv * env, jclass)
{
    chip::DeviceLayer::StackLock lock;
    ChipLogProgress(AppServer, "ManualCommissioningHelper::clearCommissionedPlayer() called");
    
    // Clear the global commissioned player reference
    gCommissionedVideoPlayer = nullptr;
    
    // Also clear from CastingStore to remove persistent cache
    matter::casting::support::CastingStore::GetInstance()->DeleteAll();
    
    ChipLogProgress(AppServer, "ManualCommissioningHelper::clearCommissionedPlayer() Cleared all commissioned player data");
}
